...
import { ResultsViewToggle } from "./car-results";
import { FilterPanel, FilterPanelHeading, FilterSummary } from "./filter-bar";
...
export function CarFiltersClient({
  ...
  function onChange(next: CarFilters) {
    startTransition(() => {
      setOptimistic(next);
  ...
  const active = countActiveFilters(optimistic);
            ...
            <Sheet>
              ...
              <SheetContent side="left" className="w-[88vw] overflow-y-auto sm:max-w-sm">
                ...
                <div className="px-4 pb-6">
                  <FilterPanel filters={optimistic} onChange={onChange} options={options} />
                </div>
              </SheetContent>
            </Sheet>
          }
          trailing={<ResultsViewToggle />}
        />
      </div>
      <div className="flex flex-1 items-stretch gap-6 pb-6">
        <aside className="hidden w-[17rem] shrink-0 lg:block">
          <div className="rounded-xl border bg-card p-5">
            <FilterPanelHeading filters={optimistic} onChange={onChange} />
            <FilterPanel filters={optimistic} onChange={onChange} options={options} />
          </div>
        </aside>
